...
__import_testtaker_btn = element(by_css("#testtaker-import>a"))
__new_test_taker_btn = element(by_css("#testtaker-new>a"))
...
def fill_label(label):
    element(by_xpath(__testtaker_field_pattern.format("Label", "input"))).set_value(label)


@step("Select language for testaker")
def select_language(language):
    element(by_xpath(__testtaker_field_pattern.format("Interface Language", "select"))).click()
    element(by_xpath(__language_select_patter.format(language))).click()


@step("Fill testtaker login")
def fill_login(login):
    element(by_xpath(__testtaker_field_pattern.format("Login", "input"))).set_value(login)


@step("Fill testtaker password")
def fill_password(password):
    element(by_xpath(__testtaker_field_pattern.format("Password", "input"))).set_value(password)
    element(by_xpath(__testtaker_field_pattern.format("Repeat password", "input"))).set_value(password)


@step("Fill user role")
def fill_role(role):
    element(by_xpath("//label[@class='elt_desc' and text()='{}']".format(role))).click()
...
def open_target_testtaker(testtaker_name):
    element(by_xpath(__testtaker_pattern.format(testtaker_name))).click()
    wait_page_reloaded()
...
def rename_testtaker(old_name, new_name):
    ...
    wait_page_reloaded()
    element(by_xpath(__testtaker_field_pattern.format("Label", "input"))).set_value(new_name)
    save_current_object()
