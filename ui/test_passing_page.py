...
def authorize_delivery(delivery_obj):
    element(by_xpath(__play_btn_pattern.format(delivery_obj.label))).click()
    ok_btn.click()
    check_popup_message("Sessions authorized")


def get_delivery_status(delivery_obj):
    return element(by_xpath(__delivery_status_pattern.format(delivery_obj.label))).text


def get_delivery_popup_message(delivery_obj):
    return element(by_xpath(__popup_message.format(delivery_obj.label))).text


def is_delivery_availiable(delivery_name):
    return element(by_xpath(__test_select_pattern.format(delivery_name.label))).is_displayed()


def select_delivery_for_passing(delivery_name):
    element(by_xpath(__test_select_pattern.format(delivery_name.label))).click()
    element(by_css(".loading-bar")).should_not(be.visible)


def choose_answer(index=None):
    if index:
        element(by_xpath("//input[@value='choice_{}']/following-sibling::span".format(index))).click()
    #element(by_xpath("//a/span[text()='Next']")).click()
    element(by_xpath("//li[@data-control='move-forward' or @data-control='move-end']/a")).click()


@step("Is test passion blocked")
def is_test_blocked():
    try:
        return element(by_xpath("//a[@class='block box']")).is_displayed()
...
def open_delivery_monitor(delivery_obj):
    element(by_xpath(__monitor_btn_pattern.format(delivery_obj.label))).click()
    wait_page_reloaded()
...
def start_session(delivery_obj):
    element(by_xpath("//h3[text()='{}]//following::span[@class='action play']".format(delivery_obj.label)))
